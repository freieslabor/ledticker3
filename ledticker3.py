class LedTicker(object):
    ...
    def get_output(self):
        ...
        """
        with open(self.ledfile, "r", encoding="utf-8") as file:
            items = [l.rstrip() for l in file]
        input_ = self.get_next_text(items)
    ...
    def add_item(self, text):
        ...
        Needs some sanity check.
        @param string text the message
        """
        with open(self.ledfile, "a", encoding="utf-8") as file:
            file.write(text + "\n")
    ...
    def items_available(self):
        ...
        """
        with open(self.ledfile, "r", encoding="utf-8") as file:
            return len([l.rstrip() for l in file])
    ...
    def get_next_text(self, items):
        ...
        @param list items all items
        @return string the text to be displayed
        """
        result = items.pop()

        with open(self.ledfile, "w", encoding="utf-8") as file:
            for item in items:
                file.write(item + "\n")
    ...
    def create_output(self, input_=""):
        ...
        """
        input_ = input_.encode("utf-8", "ignore").decode("ISO-8859-15", "ignore")
        text = "<L1><PB><FE><MC><WC><FE>"
        for char in input_:
            text += self.utf8_to_iso8859_15(ord(char))

        return "<ID%x>%s%s<E>" % (self.sign_id, text, self.calculate_checksum(text))
    ...
    def utf8_to_iso8859_15(self, utf8char):
        ...
        """
        try:
            return chr(self.ASCII2ISO8859_15[utf8char])
        except KeyError:
            return chr(utf8char)
